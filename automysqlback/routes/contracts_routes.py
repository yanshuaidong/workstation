...
# 这些函数需要从 app.py 导入
# get_db_connection, DB_CONFIG 等将在注册蓝图时传入
...
def record_daily_recommendations(date_str, get_db_connection):
    """
    记录指定日期的推荐操作到推荐记录表
    
    参数:
    - date_str: 日期字符串，格式为 'YYYY-MM-DD'
    - get_db_connection: 数据库连接函数
    """
...
def create_history_table(symbol, get_db_connection):
    """为新的主连合约创建历史数据表"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def fetch_and_store_history_with_retry(symbol, start_date, end_date, timeout_seconds, get_db_connection, max_retries=3):
    """获取并存储单个合约的历史数据（带重试）"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
@contracts_bp.route('/contracts/update-list', methods=['POST'])
    ...
    try:
        ...
        for _, row in main_contracts.iterrows():
            ...
            if table_name not in existing_tables:
                create_history_table(symbol, get_db_connection)
...
@contracts_bp.route('/contracts/list-update-log', methods=['GET'])
...
@contracts_bp.route('/history/update-all', methods=['POST'])
    ...
    try:
        ...
        def run_update_task():
            ...
            if multithread and concurrency > 1:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
                    ...
                    for symbol in symbols:
                        future = executor.submit(
                            ...
                            symbol, date_start, date_end, timeout_seconds, get_db_connection
                        )
                ...
                for symbol in symbols:
                    try:
                        fetch_and_store_history_with_retry(symbol, date_start, date_end, timeout_seconds, get_db_connection)
                    except Exception as e:
                        logger.error(f"任务执行异常: {e}")
            ...
            try:
                # 使用结束日期作为推荐记录的日期
                record_daily_recommendations(date_end, get_db_connection)
...
@contracts_bp.route('/history/retry-single', methods=['POST'])
...
@contracts_bp.route('/history/logs', methods=['GET'])
...
@contracts_bp.route('/history/recalculate-indicators', methods=['POST'])
...
@contracts_bp.route('/intraday/contracts', methods=['GET'])
...
@contracts_bp.route('/intraday/data', methods=['GET'])
...
@contracts_bp.route('/recommendations/record', methods=['POST'])
def record_recommendations():
    ...
    try:
        success = record_daily_recommendations(date_str, get_db_connection)
...
@contracts_bp.route('/recommendations/list', methods=['GET'])
