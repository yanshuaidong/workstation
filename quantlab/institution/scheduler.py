...
class InstitutionPredictScheduler:
    ...
    def setup_logging(self):
        """设置日志配置"""
        # 创建logger
        self.logger = logging.getLogger('institution_predict_scheduler')
        ...
        log_file = self.logs_dir / f"institution_predict_{current_month}.log"
    ...
    def update_log_file_if_needed(self):
        ...
        current_month = datetime.now().strftime('%Y-%m')
        expected_log_file = self.logs_dir / f"institution_predict_{current_month}.log"
...
def main():
    """主函数"""
    scheduler = InstitutionPredictScheduler()
    scheduler.run()
