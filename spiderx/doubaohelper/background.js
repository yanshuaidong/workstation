...
console.log("[Background] ========== 豆包 Helper Background Service Worker 已启动 ==========");

// ==================== 常量定义 ====================
const BACKEND_URL = 'http://localhost:1127';
const DOUBAO_URL = 'https://www.doubao.com/chat/';
const ALARM_NAME = 'doubao_daily_task';
const STORAGE_KEYS = {
...
async function handleStartScheduler() {
  try {
    ...
    // 定义执行时间点：1点、5点、9点、13点、17点、21点（与ChatGPT错开1小时）
    ...
    return {
      success: true,
      message: '定时器已启动（每天6次：1点、5点、9点、13点、17点、21点）',
...
async function executeScheduledTask(isTest = false) {
  ...
  try {
    ...
    // 3. 打开或找到豆包标签页
    console.log("[Background] 步骤2: 打开豆包标签页...");
    const tabId = await ensureDoubaoTab();
    console.log(`[Background] 豆包标签页 ID: ${tabId}`);
    ...
    if (!isReady) {
      throw new Error('Content Script 未能准备就绪，请刷新豆包页面后重试');
    }
...
async function ensureDoubaoTab() {
  ...
  if (tabs.length > 0) {
    const tab = tabs[0];
    console.log("[Background] 找到已有的豆包标签页:", tab.id);
  ...
  // 没有找到，创建新标签页
  console.log("[Background] 创建新的豆包标签页");
  const tab = await chrome.tabs.create({ url: DOUBAO_URL, active: true });
