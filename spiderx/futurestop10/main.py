...
不是所有TOP10品种都值得评分，只对搜索到"可交易事件"（6维度评分≥2分）的品种进行详细评分。

=== 可交易事件6维度评分标准 ===
每个维度满足得1分，总分≥2分即为"可交易事件"，可以启动交易！
...
⚠️ 重要原则：消息面可交易（≥2分）就可以推荐！
即使技术面暂时没有完全配合，只要事件可交易评分≥2分，就值得关注和推荐。
...
├─ 综合评分：X/12分
└─ 结论：[可交易✅(≥2分)/不可交易❌(<2分)] + [一句话说明核心逻辑]

（不可交易的品种可简化输出，只写：[品种] | X% | 6维度X/6分 | 不可交易❌：[原因]）

二、今日可执行推荐
筛选逻辑：消息面6维度≥2分即可推荐，技术面配合度决定仓位大小。

【强烈推荐 ★★★】6维度≥4分 + 技术面完全配合 → 可重仓
[品种] | 做多/做空 | 综合X/12分
事件：[核心驱动一句话]
技术：✅增仓X% ✅放量X ✅大K线X%
仓位：重仓 | 风险：[20字以内]

【推荐 ★★】6维度2-3分 + 技术面部分配合 → 可轻仓
[品种] | 做多/做空 | 综合X/12分
事件：[核心驱动一句话]
技术：[✅/❌增仓] [✅/❌放量] [✅/❌大K线]
仓位：轻仓 | 风险：[20字以内]

【可关注 ★】6维度=2分 + 技术面暂无配合 → 等确认
[品种] | 做多/做空 | 综合X/12分
事件：[核心驱动一句话]
等待：[什么信号再入场]

【无推荐】如无可交易品种，输出："今日无可交易事件驱动的标的，TOP10品种6维度评分均<2分。"

⚠️ 重要提示：
1. 必须使用浏览器搜索工具查询新闻，严禁凭空想象！
2. 消息面6维度≥2分是入场核心理由，技术面决定仓位大小！"""
