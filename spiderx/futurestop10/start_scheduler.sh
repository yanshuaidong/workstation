...
if [ -f "requirements.txt" ]; then
    echo "🔍 检查依赖..."
    $PYTHON_CMD -c "import apscheduler, pymysql, requests" 2>/dev/null
    if [ $? -ne 0 ]; then
        echo "⚠️  检测到缺少依赖，正在安装..."
...
echo "👀 日志预览："
sleep 1
tail -15 scheduler.log 2>/dev/null || tail -15 nohup.out 2>/dev/null || echo "日志文件尚未生成，请稍等..."
