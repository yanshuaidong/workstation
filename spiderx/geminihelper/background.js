...
console.log("[Background] ========== Gemini Helper Background Service Worker 已启动 ==========");

// ==================== 常量定义 ====================
const BACKEND_URL = 'http://localhost:1124';
const GEMINI_URL = 'https://gemini.google.com';
const ALARM_NAME = 'gemini_daily_task';
const STORAGE_KEYS = {
...
async function executeScheduledTask(isTest = false) {
  ...
  try {
    ...
    console.log(`[Background] Gemini 标签页 ID: ${tabId}`);
    ...
    if (!isReady) {
      throw new Error('Content Script 未能准备就绪，请刷新 Gemini 页面后重试');
    }
...
async function ensureGeminiTab() {
  ...
  if (tabs.length > 0) {
    const tab = tabs[0];
    console.log("[Background] 找到已有的 Gemini 标签页:", tab.id);
  ...
  // 没有找到，创建新标签页
  console.log("[Background] 创建新的 Gemini 标签页");
