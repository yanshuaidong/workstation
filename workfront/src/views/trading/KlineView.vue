...
import { getTradingVarietyListApi, getTradingVarietyKlineApi } from '@/api'
import {
...
export default {
  ...
  methods: {
    async fetchVarieties() {
      try {
        const res = await request.get(getTradingVarietyListApi)
        if (res.code === 0) {
    ...
    async fetchData() {
      ...
      try {
        ...
        const res = await request.get(getTradingVarietyKlineApi, {
    ...
    renderCharts(data) {
      const { dates, ohlcv, volumes, mainForce, retail, varietyName } =
    ...
    renderKline(dates, ohlcv, volumes, title) {
      ...
      this.klineInstance.setOption(buildAssistantKlineOption({ dates, ohlcv, volumes, title }), true)
    },
    renderIndex(el, instanceKey, dates, values, title, color) {
      ...
      this[instanceKey].setOption(buildAssistantIndexOption({ dates, values, title, color }), true)
    },
    resizeCharts() {
